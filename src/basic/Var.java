...
//        첫번째 명령문 - 출력문
        ...
        // 정수형
        byte a1;    // 1바이트 -> 변수를 선언(사용)한다. (8비트는 1바이트)
        short a2;   // 2바이트
        char a3;    // 2바이트
        int a4;     // 4바이트
        long a5;    // 8바이트
        ...
        a1 = 100;
        byte b1 = 100;  // 1바이트 -> 선언과 사용을 한번에 -> 초기화
        byte c1 = 100;
//        byte d1 = a1 + b1 + c1; : 이렇게 하면 에러
        a1 = -128;
        // 변수의 작성 규칙
        // 1. 대소문자를 완벽히 구분한다. a1과 A1은 다르다.
        // 2. 숫자와 _(언더바)를 활용할 수 있다.
        // 3. 한글 사용은 안 된다.
        // 4. 숫자가 제일 앞에 나올 수 없다. (영어가 먼저)
        // 5. 키워드(리저브어) 명은 단독으로 변수 명으로 사용할 수 없다.

        // 변수명을 작성할 때 자바 언어 사용자들의 문화적 특징
        // 1. 변수명은 가급적 소문자로 시작하게 작성한다.
        // 2. 변수명만 보고 어떤 종류의 데이터를 처리하는 변수인지 짐작 가능하도록 작성한다.
        // 2. ex)kor = 100;, total = kor + mate + eng;
        // 3. 두 개 이상의 영단어를 붙여서 사용할 시에는 카멜 표기법을 따른다.
        // 3. ex)myData(중간 영단어 첫글자를 대문자로 하는 게 낙타 혹 같아서 카멜 표기법)
